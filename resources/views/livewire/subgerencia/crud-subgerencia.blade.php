    ...
    <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg px-4 py-4 mx-11">
        <div class="flex items-center justify-between">
            ...
            <div class="lg:ml-40 ml-10 space-x-8">
                ...
                @if ($isOpen)
                    @include('livewire.subgerencia.subgerencia-create')
                @endif
            </div>
        </div>
        <div class="shadow overflow-x-auto border-b border-gray-700 sm:rounded-lg mt-5">
            <table class="w-full divide-y divide-gray-200 table-auto">
                ...
                <tbody class="divide-y divide-indigo-900 capitalize">
                    @foreach ($subgerencias as $item)
                        <tr class="text-sm text-center font-medium text-gray-900 capitalize"
                            wire:key="subgerencia-{{ $item->id }}">
                            <td class="px-6 py-4">
                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-indigo-900 text-white">
                                    {{ $item->id }}
                                </span>
                            </td>
                            <td class="px-6 py-4">
                                <div class="inline-flex italic items-center px-3 py-1 text-violet-500 rounded-full gap-x-2 bg-vtext-violet-100">
        ...
        @if (!$subgerencias->count())
            No existe ningun registro conincidente
        @endif
        @if ($subgerencias->hasPages())
            <div class="px-6 py-3">
                {{ $subgerencias->links() }}
            </div>
        @endif
        ...
        Livewire.on('deleteItem', id => {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    //console.log(id);
                    //Livewire.emitTo('subgerencia-main','delete',id);
