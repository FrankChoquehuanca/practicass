    ...
    <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg px-4 py-4 mx-11">
        <div class="flex items-center justify-between">
            ...
            <div class="lg:ml-40 ml-10 space-x-8">
                ...
                @if ($isOpen)
                    @include('livewire.directorio.directorio-create')
                @endif
            </div>
        </div>
        <div class="shadow overflow-x-auto border-b border-gray-700 sm:rounded-lg mt-5">
            <table class="w-full divide-y divide-gray-200 table-auto">
                <thead class="bg-indigo-900 text-white">
                    <tr class="text-center text-xs font-bold uppercase">
                        <td scope="col" class="px-6 py-3">ID</td>
                            ...
                            <td scope="col" class="px-6 py-3">relacion</td>
                            <td scope="col" class="px-6 py-3">CargoAdicional</td>
                ...
                <tbody class="divide-y divide-indigo-900 capitalize">
                    ...
                    <tr class="text-sm font-medium text-gray-900" wire:key="directorio-{{ $item->id }}">
                        <td class="px-6 py-4">
                            <span
                        ...
                        <td class="px-6 py-4 text-center">
                            @if ($item->relacion)
                                <div class="inline-flex italic items-center px-3 py-1 text-red-500 rounded-full gap-x-2 bg-red-100 mt-1">
                                   <svg  xmlns="http://www.w3.org/2000/svg"  width="24"  height="24"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round"  stroke-linejoin="round"  class="icon icon-tabler icons-tabler-outline icon-tabler-user-pin"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M8 7a4 4 0 1 0 8 0a4 4 0 0 0 -8 0" /><path d="M6 21v-2a4 4 0 0 1 4 -4h2.5" /><path d="M21.121 20.121a3 3 0 1 0 -4.242 0c.418 .419 1.125 1.045 2.121 1.879c1.051 -.89 1.759 -1.516 2.121 -1.879z" /><path d="M19 18v.01" /></svg>
                                    gerencia:
                                    <h2 class="text-sm font-normal not-italic ">{{ $item->relacion->gerencia->nombre }}</h2>
                                </div>
                                @if ($item->relacion->subgerencia)
                                <div class="inline-flex italic items-center px-3 py-1 text-violet-500 rounded-full gap-x-2 bg-violet-100 mt-1">
                                   <svg  xmlns="http://www.w3.org/2000/svg"  width="24"  height="24"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round"  stroke-linejoin="round"  class="icon icon-tabler icons-tabler-outline icon-tabler-user-pin"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M8 7a4 4 0 1 0 8 0a4 4 0 0 0 -8 0" /><path d="M6 21v-2a4 4 0 0 1 4 -4h2.5" /><path d="M21.121 20.121a3 3 0 1 0 -4.242 0c.418 .419 1.125 1.045 2.121 1.879c1.051 -.89 1.759 -1.516 2.121 -1.879z" /><path d="M19 18v.01" /></svg>
                                    Subgerencia:
                                    <h2 class="text-sm font-normal not-italic">{{ $item->relacion->subgerencia->nombre }}</h2>
                                </div>
                                @endif
                                @if ($item->relacion->departamento)
                                <div class="inline-flex italic items-center px-3 py-1 text-blue-500 rounded-full gap-x-2 bg-blue-100 mt-1">
                                   <svg  xmlns="http://www.w3.org/2000/svg"  width="24"  height="24"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round"  stroke-linejoin="round"  class="icon icon-tabler icons-tabler-outline icon-tabler-user-pin"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M8 7a4 4 0 1 0 8 0a4 4 0 0 0 -8 0" /><path d="M6 21v-2a4 4 0 0 1 4 -4h2.5" /><path d="M21.121 20.121a3 3 0 1 0 -4.242 0c.418 .419 1.125 1.045 2.121 1.879c1.051 -.89 1.759 -1.516 2.121 -1.879z" /><path d="M19 18v.01" /></svg>
                                    Departamento:
                                    <h2 class="text-sm font-normal text not-italic">{{ $item->relacion->departamento->nombre }}</h2>
                                </div>
                                @endif
                                @if ($item->relacion->unidad)
                                <div class="inline-flex italic items-center px-3 py-1 text-green-500 rounded-full gap-x-2 bg-green-100 mt-1">
                                   <svg  xmlns="http://www.w3.org/2000/svg"  width="24"  height="24"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round"  stroke-linejoin="round"  class="icon icon-tabler icons-tabler-outline icon-tabler-user-pin"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M8 7a4 4 0 1 0 8 0a4 4 0 0 0 -8 0" /><path d="M6 21v-2a4 4 0 0 1 4 -4h2.5" /><path d="M21.121 20.121a3 3 0 1 0 -4.242 0c.418 .419 1.125 1.045 2.121 1.879c1.051 -.89 1.759 -1.516 2.121 -1.879z" /><path d="M19 18v.01" /></svg>
                                    Unidad:
                                    <h2 class="text-sm font-normal not-italic">{{ $item->relacion->unidad->nombre }}</h2>
                                </div>
                                @endif
                                @if ($item->relacion->area)
                                <div class="inline-flex italic items-center px-3 py-1 text-sky-500 rounded-full gap-x-2 bg-sky-100 mt-1">
                                   <svg  xmlns="http://www.w3.org/2000/svg"  width="24"  height="24"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round"  stroke-linejoin="round"  class="icon icon-tabler icons-tabler-outline icon-tabler-user-pin"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M8 7a4 4 0 1 0 8 0a4 4 0 0 0 -8 0" /><path d="M6 21v-2a4 4 0 0 1 4 -4h2.5" /><path d="M21.121 20.121a3 3 0 1 0 -4.242 0c.418 .419 1.125 1.045 2.121 1.879c1.051 -.89 1.759 -1.516 2.121 -1.879z" /><path d="M19 18v.01" /></svg>
                                    Area:
                                    <h2 class="text-sm font-normal not-italic">{{ $item->relacion->area->nombre }}</h2>
                                </div>
                                @endif
                            @else
                                <div class="flex justify-center items-center h-full rounded-full italic">
                                    <span class="text-gray-400 text-center">N/A</span>
                                </div>
                            @endif
                        </td>
                        <td class="px-6 py-4 text-center capitalize">
                            @if ($item->cargoadicional)
                            <div class="inline-flex items-center px-3 py-1 text-gray-500 rounded-full gap-x-2 bg-gray-100">
                                <svg  xmlns="http://www.w3.org/2000/svg"  width="24"  height="24"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round"  stroke-linejoin="round"  class="icon icon-tabler icons-tabler-outline icon-tabler-user-plus"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M8 7a4 4 0 1 0 8 0a4 4 0 0 0 -8 0" /><path d="M16 19h6" /><path d="M19 16v6" /><path d="M6 21v-2a4 4 0 0 1 4 -4h4" /></svg>                                <h2 class="text-sm font-normal">{{ $item->cargoadicional->nombre }}</h2>
                            </div>
                            @else
                                <div class="flex justify-center items-center h-full italic">
                                    <span class="text-gray-400 text-center">N/A</span>
                                </div>
                            @endif
                        </td>
    ...
    Livewire.on('deleteItem', id => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                //console.log(id);
                //Livewire.emitTo('directorio-main','delete',id);
